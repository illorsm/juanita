...
local Library = { 
    ...
    local Themes = {
        ["Preset"] = {
            ...
            ["Risky"] = Color3.fromRGB(227, 37, 41),
            ["Buggy"] = Color3.fromRGB(227, 177, 27),
        }
    }
    ...
    Library.MakeResizeable = function(Self, Minimum)
        ...
        local MakeEdge = function(Name, Position, Size)
            ...
            Button:OnHover(function()
                ...
                Button:Tween({BackgroundTransparency = 0.35})
            end, function()
                if Resizing then return end 
        ...
        local BeginResizing = function(Side)
            ...
            for Index, Value in Edges do 
                Value.Button:Tween({BackgroundTransparency = (Value.Side == Side) and 0 or 1})
            end
        end
    ...
    Library.UpdateNotifications = function(Self)
        local Corner = Flags["NotificationPosition"] or "Top Left"
        ...
        local ScreenSize = Library.NotifHolder.Instance.AbsoluteSize
        local Accumulated = 0
        local AnchorX = IsLeft and 0 or 1
        local AnchorY = IsTop and 0 or 1
    
        for _, Notif in Library.Notifications do
            local Instance = Notif.Items["Notification"].Instance
            local Height = Instance.AbsoluteSize.Y
    
            local PosY

            if IsTop then
                PosY = GuiInset + Padding + 5 + Accumulated
            else
                PosY = ScreenSize.Y - Padding - 5 - Accumulated
            end
    
            local PosX = IsLeft and Padding or -Padding
    
            Instance.AnchorPoint = Vector2.new(AnchorX, AnchorY)
            Library:Tween({Position = UDim2.new(AnchorX == 1 and 1 or 0, PosX, 0, PosY)}, TweenInfo.new(0.85, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), Instance)
    
            Accumulated = Accumulated + Height + Spacing
        end
    end
    ...
    do
        Library.LerpColor = function(Self, A, B, T)
            return A:Lerp(B, math.clamp(T, 0, 1))
        end

        Library.RainbowTime = 0

        Library:Connect(RunService.RenderStepped, function(dt)
            Library.RainbowTime += dt
        
            for Colorpicker in Library.AnimatedColorpickers do
                local hue = (Library.RainbowTime * Colorpicker.AnimationSpeed * 0.2) % 1
        
                Colorpicker:UpdateRainbow(
                    Color3.fromHSV(
                        hue,
        ...
        Library.SetColorpickerAnimating = function(Self, Colorpicker, Enabled)
            if Enabled then
        ...
        Library.CreateColorpicker = function(Self, Data)
            ...
            local Items = { } do 
                Items["ColorpickerButtonOutline"] = Library:Create("TextButton", {
                    Name = "\0",
                ...
                Items["ColorpickerButton"] = Library:Create("TextButton", {
                    ...
                    FontFace = Library.Font,
                    TextSize = Library.FontSize,
                    Parent = Items["ColorpickerButtonOutline"].Instance,
                    TextColor3 = Color3.fromRGB(0, 0, 0),
                    Text = "",
                ...
                Items["ColorCheckers"] = Library:Create("ImageLabel", {
                    Name = "\0",
                    Parent = Items["ColorpickerButton"].Instance,
            ...
            function Colorpicker:Update(IsFromAlpha)
                ...
                Items["ColorCheckers"]:Tween({ImageTransparency = Flags[Colorpicker.Flag].Transparency})
            ...
            function Colorpicker:UpdateRainbow(Color)
                Colorpicker.Color = Color
                Colorpicker.HexValue = Color:ToHex()
            
                local Items = Colorpicker.Items
            
                Items.ColorpickerButton.Instance.BackgroundColor3 = Color
            
                if Colorpicker.IsOpen then
                    Items.CurrentColor.Instance.BackgroundColor3 = Color
                    Items.Alpha.Instance.BackgroundColor3 = Color
                    Items.Palette.Instance.BackgroundColor3 = Color3.fromHSV(Colorpicker.Hue,1,1)
                end
            
                local Flag = Flags[Colorpicker.Flag]
            
                if Flag then
                    Flag.Color = Color
                    Flag.HexValue = self.HexValue
                    Flag.Alpha = self.Alpha
                    Flag.Transparency = 1 - self.Alpha
                end
            
                -- i have to do this because if theres rainbow and if a callback updates esp, chams etc.... it'll be super laggy
                if Data.Callback then
                    local t = os.clock()
            
                    if not Colorpicker.LastCallback or t - Colorpicker.LastCallback > 0.05 then
                        Colorpicker.LastCallback = t
            ...
            local RainbowToggle = Library:Toggle({
                Name = "Rainbow",
                Flag = Colorpicker.Flag.."Rainbow",
                Default = false,
                Parent = Items["ColorpickerWindow"],
                Callback = function(Value)
                    Colorpicker.AnimationMode = Value and "Rainbow" or "None"
                    Library:SetColorpickerAnimating(Colorpicker, Value)
                end
            })

            RainbowToggle.Items.Toggle.Instance.AnchorPoint = Vector2.new(0, 1)
            RainbowToggle.Items.Toggle.Instance.Position = UDim2.new(0, 8, 1, -50)

            local RainbowSpeedSlider = Library:Slider({
                Name = "Speed",
                Flag = Colorpicker.Flag.."Speed",
                Default = 0.20,
                Decimals = 0.01,
                Min = 0,
                Max = 1,
                Parent = Items["ColorpickerWindow"],
                Callback = function(Value)
                    Colorpicker.AnimationSpeed = Value
                end
            })

            RainbowSpeedSlider.Items.Slider.Instance.AnchorPoint = Vector2.new(0, 1)
            RainbowSpeedSlider.Items.Slider.Instance.Position = UDim2.new(0, 8, 1, -10)
            RainbowSpeedSlider.Items.Slider.Instance.Size = UDim2.new(1, -16, 0, 30)
        ...
        Library.Notification = function(Self, Name, Duration, Color)
            ...
            local Items = {} do
                ...
                Items["Notification"].Instance.AnchorPoint = Vector2.new(AnchorX, AnchorY)
            ...
            Items["Notification"].Instance.Position = UDim2.new(AnchorX == 1 and 1 or 0, OffX, 0, AnchorY == 1 and (Library.NotifHolder.Instance.AbsoluteSize.Y + Padding + 5) or (GuiInset + Padding + 5))
            ...
            task.spawn(function()
                ...
                if IsLeft then
                    OutX = -(Width + Padding + 20)
                else
                    OutX = Width + Padding + 20
                end

                Items["Notification"]:Tween({Position = UDim2.new(AnchorX == 1 and 1 or 0, OutX, 0, Items["Notification"].Instance.Position.Y.Offset)}, NotifTweenInfo)
        ...
        Library.Tooltip = function(Self, Text)
            ...
            local Items = { } do 
                Items["TooltipOutline"] = Library:Create("Frame", {
                    Name = "\0",
                    Parent = Library.Holder.Instance,
                    Size = UDim2.new(0, 100, 0, 25),
                    Position = UDim2.new(0, 982, 0, 437),
                    BorderSizePixel = 0,
                    ClipsDescendants = true,
                    AutomaticSize = Enum.AutomaticSize.X,
                ...
                Items["TooltipInline"] = Library:Create("Frame", {
                    Name = "\0",
                    Parent = Items["TooltipOutline"].Instance,
                    Size = UDim2.new(1, 0, 1, 0),
                    BorderSizePixel = 0,
                    AutomaticSize = Enum.AutomaticSize.X,
                ...
                Items["Tooltip"] = Library:Create("Frame", {
                    Name = "\0",
                    Parent = Items["TooltipInline"].Instance,
                    Size = UDim2.new(1, 0, 1, 0),
                    BorderSizePixel = 0,
                    AutomaticSize = Enum.AutomaticSize.X,
                ...
                Items["Text"] = Library:Create("TextLabel", {
                    ...
                    TextColor3 = Color3.fromRGB(208, 207, 227),
                    Text = Text,
                    AnchorPoint = Vector2.new(0, 0.5),
                    BackgroundTransparency = 1,
            ...
            local GetSize = function()
                ...
                local AbsSize = Items["TooltipOutline"].Instance.AbsoluteSize
                local AbsSize2 = Items["TooltipInline"].Instance.AbsoluteSize
                local AbsSize3 = Items["Tooltip"].Instance.AbsoluteSize

                Items["TooltipOutline"].Instance.AutomaticSize = Enum.AutomaticSize.None
                Items["TooltipInline"].Instance.AutomaticSize = Enum.AutomaticSize.None
                Items["Tooltip"].Instance.AutomaticSize = Enum.AutomaticSize.None

                Items["TooltipOutline"].Instance.Size = UDim2.new(0, AbsSize.X, 0, AbsSize.Y)
                Items["TooltipInline"].Instance.Size = UDim2.new(0, AbsSize2.X, 0, AbsSize2.Y)
                Items["Tooltip"].Instance.Size = UDim2.new(0, AbsSize3.X, 0, AbsSize3.Y)

                return AbsSize
            end

            local FadeDescendants = function(Bool, Instant)
                for _, Value in Items do
                    ...
                    if Value.Instance:IsA("Frame") then
                        if Instant then
                            Value.Instance.BackgroundTransparency = Bool and 1 or 0
                        else
                            Value:Tween({BackgroundTransparency = Bool and 1 or 0})
                        end
                    elseif Value.Instance:IsA("TextLabel") then
                        if Instant then
                            Value.Instance.TextTransparency = Bool and 1 or 0
                        else
            ...
            FadeDescendants(true, true)

            local Size = GetSize()
            Items["TooltipOutline"].Instance.Size = UDim2.new(0, 0, 0, Size.Y)
            ...
            Self:OnHover(function()
                ...
                Items["TooltipOutline"]:Tween({Size = UDim2.new(0, Size.X, 0, Size.Y)})
                FadeDescendants(false)
        ...
        Library.Toggle = function(Self, Params)
            ...
            local Toggle = {
                ...
                Status = Params.Status or Params.status or "Neutral",
            ...
            local Items = { } do 
                ...
                Items["Text"] = Library:Create("TextLabel", {
                    ...
                    BorderSizePixel = 0,
                    AutomaticSize = Enum.AutomaticSize.XY
                }):AddToTheme({TextColor3 = Toggle.Status == "Neutral" and 'Text' or Toggle.Status == "Risky" and 'Risky' or 'Buggy'})       
        ...
        Library.Button = function(Self, Params)
            ...
            local Items = { } do 
                ...
                Items["RealButton"]:Tooltip(Button.Tooltip)
        ...
        Library.List = function(Self, Params)
            ...
            local Items = { } do 
                ...
                Items["List"]:Tooltip(List.Tooltip)
